...
# ---- Wordlists for bruteforce (if enabled) ----
DEFAULT_WORDLIST = [
...
def fetch_and_extract_domains(url, session, base_domain, visited_files, queue_for_fetch):
    ...
    try:
        ...
        elif any(ct in content_type for ct in ['text/css', 'application/javascript', 'text/javascript', 'application/json', 'text/xml', 'application/xml']):
            domains.update(extract_domains_from_text(text, base_domain))
        # Even if it's an image or font, we might still get domains from the URL itself
...
def run_domain_crawler(task_id, start_url, max_pages, max_depth, enable_bruteforce, wordlist, extensions, threads):
    ...
    try:
        ...
        # ---- Fetch all discovered assets (CSS, JS, etc.) ----
        asset_count = 0
        ...
        if enable_bruteforce and not task.get('cancelled', False):
            ...
            with ThreadPoolExecutor(max_workers=threads) as executor:
                ...
                for future in as_completed(futures):
                    ...
                    bf_checked += 1
                    if url:
                        # Fetch this discovered file if it's text-based to extract domains
                        if any(url.endswith(ext) for ext in ['.css', '.js', '.json', '.xml', '.txt', '.php', '.html']):
                            ...
                            # Even if it's an image, we might still get domains from its URL (e.g., CDN)
