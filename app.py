...
def create_app():
    ...
    def not_found(e):
        # Return a simple JSON for missing API routes, but for normal pages we let it fall through
        if request.path.startswith('/api') or request.path == '/get_tasks':
            return jsonify({'error': 'Not found'}), 404
        # For favicon, just return 204 (no content)
        if request.path == '/favicon.ico':
            return '', 204
        # Otherwise, re-raise the 404 (will show default error page)
    ...
    # Optional: serve favicon from static folder if exists
